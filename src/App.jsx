...
function safeJSONParse(text) {
  ...
  // Fix unquoted array values: patterns like [value1, unquoted value2, value3]
  // Match array contents and quote unquoted values (containing spaces, parens, math symbols)
  fixed = fixed.replace(/\[([^\]]*)\]/g, (fullMatch, inner) => {
    ...
    for (let i = 0; i < inner.length; i++) {
      ...
      if (c === '\\' && inStr) { current += c; esc = true; continue; }
      if (c === '"') { inStr = !inStr; current += c; continue; }
      if (c === ',' && !inStr) { items.push(current.trim()); current = ''; continue; }
      current += c;
    }
    if (current.trim()) items.push(current.trim());

    // Quote any unquoted values
    const fixedItems = items.map(item => {
      if (/^".*"$/.test(item)) return item; // already quoted
      if (/^-?\d+(\.\d+)?$/.test(item)) return item; // number
      if (/^(true|false|null)$/.test(item)) return item; // boolean/null
      // Otherwise it's an unquoted string — wrap in quotes, escaping any inner quotes
  ...
  try { return JSON.parse(result); } catch (e) {
    console.warn('Safe JSON parse failed, falling back to mock paper:', e);
    throw e;
  }
}
...
async function generatePaper(assignment, seed = '') {
  ...
  try {
    ...
    let paper;
    try {
      paper = safeJSONParse(raw);
    } catch (parseErr) {
      console.error('JSON parse failed. Full raw response:', raw);
      throw parseErr;
    }
...
const Ic = {
  ...
  File:     () => (<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M14 2H6a4 4 0 00-2 2v16a3 3 0 003-3H2z"/><polyline points="14 2 14 8 20 8"/></svg>),
